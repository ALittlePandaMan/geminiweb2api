...
def _log_model_request_debug(model: Model | AvailableModel) -> None:
    logger.debug(
        "Resolved Gemini request model: "
        f"model_name={getattr(model, 'model_name', str(model))!r} "
        f"display_name={getattr(model, 'display_name', None)!r} "
        f"is_available={getattr(model, 'is_available', None)!r} "
        f"model_header={getattr(model, 'model_header', {})!r}"
    )
...
class GeminiClient(ChatMixin, GemMixin, ResearchMixin):
    ...
    async def _generate(
        ...
        **kwargs,
        ...
        if self.verbose:
            _log_model_request_debug(model)
            ...
            try:
                ...
                async with self.client.stream(
                    ...
                    **kwargs,
                    ...
                    is_final_chunk = False  # Check if this turn is saved to history and marked complete or still pending.
